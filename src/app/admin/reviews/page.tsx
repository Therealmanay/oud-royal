...
import { Star, CheckCircle, XCircle, MessageSquare, Plus, Trash2, Loader2 } from 'lucide-react'
...
export default function AdminReviewsPage() {
  ...
  const [form, setForm] = useState({ name: '', productId: '', rating: '5', comment: '', isApproved: true })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: '', productId: '', rating: '5', comment: '', isApproved: true })
      setShowForm(false)
      fetchData()
    } catch (error: any) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            {reviews.length} avis • {approvedCount} approuvés • {pendingCount} en attente
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card-dark p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Un parfum incroyable, la tenue est exceptionnelle..."
                ...
                <tr key={review.id} className="border-b border-dark-border/30 hover:bg-dark-card-hover transition-colors">
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all"
                    >
                      <Trash2 size={14} />
                    </button>
                  </td>
